...
import { CartCtx, type CartContext, type CartLine } from './_app';
...
export default function Home() {
  const { cart, setCart, lang } = useContext<CartContext>(CartCtx);
  ...
  const t = (nb: string, en: string): string => (lang === 'nb' ? nb : en);

  const add = (m: Merchant, item: { id: string; name: string; price: number }) => {
    const exist = (cart as CartLine[]).find((c) => c.itemId === item.id && c.merchantId === m.id);
    if (exist) {
      exist.qty += 1;
      setCart([...(cart as CartLine[])]);
    } else {
      setCart([
        ...(cart as CartLine[]),
        { merchantId: m.id, merchantName: m.name, itemId: item.id, name: item.name, price: item.price, qty: 1 },
      ]);
    }
  };
  ...
  const groupedByMerchant = useMemo(() => {
    ...
    (cart as CartLine[]).forEach((line) => {
      if (!acc[line.merchantId]) {
        const km =
          (merchantsData as Merchant[]).find((m) => m.id === line.merchantId)?.distance_km ?? 0;
        acc[line.merchantId] = { merchantId: line.merchantId, merchantName: line.merchantName, items: [], km };
      }
